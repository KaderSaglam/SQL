/*============================== JOIN İSLEMLERİ ===============================
    ...
    3) RIGHT JOIN: Sadece Ikinci tabloda olan tum sonuclari gosterir
       full  JOIN=(left join + union + right join) Tablodaki tum sonuclari gosterir -- mysql de full join olmaz
==============================================================================*/   
...
/* -----------------------------------------------------------------------------
  ORNEK1: Iki Tabloda sirket_id'si ayni olanlarin sirket_isim, siparis_id ve 
  siparis_tarihleri listeleyen bir sorgu yaziniz.
------------------------------------------------------------------------------*/

select sir.sirket_isim, sip.siparis_id, sip.siparis_tarihi, sip.sirket_id
from sirketler sir
inner join siparisler sip -- sirketler tablosuyla siparisler tablosunu birlestir ama ortklari al
...
------------------------------------------------------------------------------*/   
    ...
    from sirketler as sir-- yanci önemsiz solda
    right join siparisler as sip -- ana tablo sagda
    on sir.sirket_id=sip.sirket_id;
...
on p.bolum_id=b.bolum_id -- baglanabilecekleri yer on  birlestirme
where  bolum_isim in('satis', 'muhasebe')
order by b.bolum_isim, p.personel_isim;
-- baglama isi joinlerde on keywordu ile yapilir
/* -----------------------------------------------------------------------------
...
------------------------------------------------------------------------------*/  
select b.bolum_isim, personel_isim, p.maas
from bolumler b join personel p
on p.bolum_id=bolum_id
where b.bolum_isim in('satis','mudurluk') and p.mmas>2000
order by b.bolum_isim,p.personel_isim;

/* -----------------------------------------------------------------------------
  ORNEK5: MUDUR'u Harun veya Emine olan personelin bolumlerini,isimlerini,  
  maaslarini ve ayrica MUDUR isimlerini 
  (emine kimin müdürüyse onun satirinda yazsin)  (bu sarti en son coz)
